...
<div class="page">
  ...
  <header>
    ...
    <div class="header-right">
      <div class="header-badge"><span class="dot" style="background:#7c3aed;"></span>Pipeline + Carteira · 155 proj · 135 contr</div>
    </div>
  </header>
...
var _seg=null;
function badge(e){var c=ec[e]||["#f1f5f9","#334155"];return'<span style="background:'+c[0]+';color:'+c[1]+';font-size:10px;font-weight:600;padding:2px 8px;border-radius:99px;white-space:nowrap;">'+e+'</span>';}
function agePill(d){if(d===null||d===undefined)return"—";var c,bg;if(d>60){c="#dc2626";bg="#fef2f2";}else if(d>=30){c="#d97706";bg="#fffbeb";}else{c="#16a34a";bg="#f0fdf4";}return'<span style="display:inline-flex;align-items:center;gap:4px;background:'+bg+';color:'+c+';font-size:10px;font-weight:700;padding:2px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:'+c+';"></span>'+d+'d</span>';}
function brf(v){if(!v||v===0)return'<span style="color:#d1d5db;">—</span>';return"R$ "+v.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});}
function renderP(info,color){
  var rows=(info.projetos||[]).map(function(p,i){
    var bg=i%2===0?"#ffffff":"#fafafa";
    var nc=p.tipo==="fechado"?"#059669":"#2563eb";
    return'<tr style="background:'+bg+';border-bottom:1px solid #f3f4f6;">'+
      '<td style="padding:9px 14px;font-family:monospace;font-size:11px;font-weight:600;color:'+nc+';">'+p.np+'</td>'+
      ...
      '<td style="padding:9px 14px;text-align:right;font-size:12px;font-weight:700;">'+brf(p.vt)+'</td>'+
      '<td style="padding:9px 14px;text-align:center;">'+agePill(p.dias)+'</td></tr>';
  }).join("");
  document.getElementById("wtp-dd-tbody").innerHTML=rows;
  document.getElementById("wtp-dd-thead").innerHTML='<tr style="background:#f8fafc;"><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">NP</th><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Cliente</th><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Etapa</th><th style="padding:9px 14px;text-align:right;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Valor Total</th><th style="padding:9px 14px;text-align:center;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Idade</th></tr>';
}
function renderC(info,color){
  var rows=(info.contratos||[]).map(function(p,i){
    var bg=i%2===0?"#ffffff":"#fafafa";
    var nc=p.cancelado?"#9ca3af":"#059669";
    return'<tr style="background:'+bg+';border-bottom:1px solid #f3f4f6;opacity:'+( p.cancelado?"0.55":"1")+'">'+
      '<td style="padding:9px 14px;font-family:monospace;font-size:11px;font-weight:600;color:'+nc+';">'+p.np+'</td>'+
      '<td style="padding:9px 14px;font-size:12px;font-weight:500;color:#111827;">'+p.cliente+'</td>'+
      '<td style="padding:9px 14px;">'+badge(p.etapa)+'</td>'+
      '<td style="padding:9px 14px;text-align:center;font-size:11px;font-weight:600;color:#374151;">'+p.vencimento+'</td>'+
      '<td style="padding:9px 14px;text-align:right;font-size:12px;font-weight:600;color:#7c3aed;">'+brf(p.mrr)+'</td>'+
      '<td style="padding:9px 14px;text-align:right;font-size:12px;font-weight:600;color:#0891b2;">'+brf(p.arr)+'</td></tr>';
  }).join("");
  document.getElementById("wtp-dd-tbody").innerHTML=rows;
  document.getElementById("wtp-dd-thead").innerHTML='<tr style="background:#f8fafc;"><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">NP</th><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Cliente</th><th style="padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Etapa</th><th style="padding:9px 14px;text-align:center;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#9ca3af;border-bottom:1px solid #e5e7eb;">Vencimento</th><th style="padding:9px 14px;text-align:right;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#7c3aed;border-bottom:1px solid #e5e7eb;">MRR</th><th style="padding:9px 14px;text-align:right;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#0891b2;border-bottom:1px solid #e5e7eb;">ARR</th></tr>';
}
window.wtpDrilldown=function(seg){
  _seg=seg;
  var info=wtp[seg];if(!info)return;
  ...
  document.getElementById("wtp-dd-title").innerHTML='<span style="color:'+color+';">'+seg+'</span>';
  document.getElementById("wtp-dd-sub").textContent=(info.projetos||[]).length+" projetos · "+(info.contratos||[]).length+" contratos · "+info.n_cli_so_contr+" clientes só na carteira";
  var bp=document.getElementById("wtp-tab-proj"),bc=document.getElementById("wtp-tab-contr");
  bp.style.borderBottom="2px solid "+color;bp.style.color="#111827";
  bc.style.borderBottom="2px solid transparent";bc.style.color="#9ca3af";
  renderP(info,color);
...
window.wtpTab=function(tab){
  if(!_seg)return;
  var info=wtp[_seg];var color=sc[_seg]||"#6b7280";
  var bp=document.getElementById("wtp-tab-proj"),bc=document.getElementById("wtp-tab-contr");
  if(tab==="proj"){
    bp.style.borderBottom="2px solid "+color;bp.style.color="#111827";
    bc.style.borderBottom="2px solid transparent";bc.style.color="#9ca3af";
    ...
    bc.style.borderBottom="2px solid "+color;bc.style.color="#111827";
    bp.style.borderBottom="2px solid transparent";bp.style.color="#9ca3af";
    renderC(info,color);
  }
};
...
document.querySelectorAll(".wtp-card").forEach(function(c){
  c.addEventListener("mouseenter",function(){if(c.style.opacity!=="0.4")c.style.boxShadow="0 4px 16px rgba(0,0,0,.07)";});
  c.addEventListener("mouseleave",function(){if(c.style.opacity!=="0.4")c.style.boxShadow="none";});
});
})();
</script>
</div>
