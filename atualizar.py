...
import individual   # painéis de Visão Individual por gestor
...
def preencher(template, pip, gest, res, dados_ind=None):
    ...
    # ── Visão Individual por gestor ──────────────────────────────────────────
    if dados_ind:
        t = t.replace('{{IND_BOTOES}}',  individual.gerar_botoes(dados_ind))
        t = t.replace('{{IND_PAINEIS}}', individual.gerar_paineis(dados_ind, hoje, date.today()))
    else:
        t = t.replace('{{IND_BOTOES}}', '').replace('{{IND_PAINEIS}}', '')
...
def main():
    ...
    ap.add_argument('--base',      default=None,  help='Base_de_Clientes.xlsx (habilita as abas de gestor)')
    ...
    dados_ind = None
    if args.base:
        print("\n👤 Calculando Visão Individual por gestor...")
        dados_ind = individual.calcular_individual(args.projetos, args.contratos, args.base)
        for slug, _, _, _ in individual.GESTORES_PAINEL:
            d = dados_ind[slug]
            print(f"  {d['curto']:<9} clientes={len(d['clientes']):>2} · leads={len(d['leads']):>2} · "
                  f"contratos={len(d['contratos']):>2} · agPF={len(d['agpf'])} · fechados={len(d['fechados']):>2}")

    print("\n🔧 Preenchendo template...")
    with open(args.template, 'r', encoding='utf-8') as f:
        template = f.read()

    worker = preencher(template, pip, gest, res, dados_ind)

    print("\n✅ Validando...")
    issues = validar(worker, pip, gest)

    # Sintaxe JS — sem isto, um worker quebrado passa despercebido até o deploy
    try:
        import subprocess
        r = subprocess.run(['node', '--input-type=module', '--check'],
                           input=worker, capture_output=True, text=True, timeout=60)
        if r.returncode != 0:
            issues.append('  ✘ SINTAXE JS INVÁLIDA — NÃO FAÇA DEPLOY:\n     '
                          + r.stderr.strip().split(chr(10))[0][:160])
        else:
            print("  ✔ sintaxe JavaScript válida")
    except FileNotFoundError:
        print("  ⚠ node nao encontrado — sintaxe JS NÃO verificada")
    except Exception as e:
        print(f"  ⚠ checagem de sintaxe falhou: {e}")

    for slug in ('anderson', 'talita', 'ines', 'pedro'):
        if f'id="tab-{slug}"' not in worker:
            issues.append(f'  ✘ painel do gestor ausente: tab-{slug}')
    if worker.count('function loginPage') != 1:
        issues.append(f'  ✘ loginPage duplicada ({worker.count("function loginPage")}×)')
    if issues:
